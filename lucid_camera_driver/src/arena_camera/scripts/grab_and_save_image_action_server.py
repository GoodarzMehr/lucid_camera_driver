...
class GrabAndSaveImageActionServers():
    """
    This node provides an action server that extends the 'grab_images_raw' and
    'grab_images_rect' action servers from the arena_camera_node. The new
    action servers are named 'grab_and_save_image_raw' and
    'grab_and_save_image_rect'. They have basically the same action goal as
    'GrabImagesAction' in case of grabbing only one image, but they extend
    the above goal with a string describing the full storage path and name of
    ...
    def __init__(self):
        ...
        if not camera_name:
            rospy.logwarn("No camera name given! Assuming 'arena_camera_node'"
                          " as camera name.")
        ...
        if self._grab_imgs_raw_ac.wait_for_server(rospy.Duration(10.0)):
            self._grab_and_save_img_raw_as = SimpleActionServer("~grab_and_save_image_raw",
        ...
        if self._grab_imgs_rect_ac.wait_for_server(rospy.Duration(2.0)):
            self._grab_and_save_img_rect_as = SimpleActionServer("~grab_and_save_image_rect",
                                                                 GrabAndSaveImageAction,
    ...
    def grab_and_save_img_execute_cb(self, grab_and_save_img_goal,
        ...
        if grab_imgs_result is not None and grab_imgs_result.success:
            ...
            try:
                ...
                rospy.logerr('Error converting img_msg to cv_img: ' +
                             str(exception))
